...
# 데이터셋 경로
dataset_path = "yolov5/helmet/train"
...
transform = A.Compose([
    ...
    A.Rotate(limit=45, p=0.2),  # 이미지를 최대 45도 회전
    A.Blur(blur_limit=3, p=0.2),  # 이미지 블러 처리
    # 다른 증강 방식 추가 가능
...
for img_file in os.listdir(images_path):
    ...
    # 증강된 이미지 저장
    aug_img_file = os.path.join(augmented_images_path, f"hij_" + img_file)
    cv2.imwrite(aug_img_file, cv2.cvtColor(augmented_image, cv2.COLOR_RGB2BGR))

    # 증강된 레이블 저장
    aug_txt_file = os.path.join(augmented_labels_path, f"hij_" + txt_file)
